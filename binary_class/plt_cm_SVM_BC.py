...
Confusion Matrix plot for SVM for LOOCV
"""
import os
import numpy as np
...
path = os.getcwd()
result_path = path + '/SA-TUNING/RESULTS/SVM/' 
...
plt.savefig(result_path+"/loocv_cm_svm_binaryclass.jpg", format='jpg', dpi=300)
plt.savefig(result_path+"loocv_cm_svm_binaryclass.eps", format='eps', dpi=300)
...
plt.savefig(result_path+"roc_svm_binaryclass.jpg", format='jpg', dpi=300)
plt.savefig(result_path+"roc_svm_binaryclass.eps", format='eps', dpi=300)
plt.show()
